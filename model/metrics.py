...
class CTCLayer(layers.Layer):
    ...
    def wer(self, y_true, y_pred, pred_sequence_length, true_sequence_length):
        batch_len = tf.cast(tf.shape(y_true)[0], dtype="int64")

        pred_codes_dense = K.ctc_decode(y_pred, tf.squeeze(pred_sequence_length, axis=-1), greedy=True)
        pred_codes_dense = tf.squeeze(tf.cast(pred_codes_dense[0], tf.int64), axis=0)
        ...
        distances = tf.edit_distance(pred_codes_sparse, label_sparse, normalize=False)

        count_chars = len(idx)

        correct_words_amount = tf.reduce_sum(tf.cast(tf.not_equal(distances, 0), tf.float32))

        return tf.divide(tf.cast(correct_words_amount, tf.float32), tf.cast(count_chars, tf.float32), name='WER')
    ...
    def call(self, y_true, y_pred):
        ...
        wer = self.wer(y_true, y_pred, input_length, label_length)

        self.add_metric(cer, name='cer')
        self.add_metric(accuracy, name='accuracy')
        self.add_metric(wer, name='wer')
